...
- **Early validation** - Config checked before processing
- **Clear guidance** - Instructions for getting API token
- **Git security** - `.env` excluded from git (with push protection detection)

### Performance Characteristics

**Test Case:** 100 issues across 5 projects in 2 teams

- **Cold run (no cache):** ~45 seconds
  - 1 team fetch
  - 2 project fetches
  - 100 issue creates

- **Warm run (cached):** ~30 seconds
  - 0 team fetches (cached)
  - 0 project fetches (cached)
  - 100 issue creates

**Bottleneck:** Linear's MCP server response time (~250-300ms per issue creation)

**Future optimization opportunity:** Parallel issue creation (currently sequential for clearer progress reporting)
...
1. **Token Protection**
   ...
   - Support for token files with restricted permissions

2. **Input Validation**
   - CSV injection prevention (Pydantic validation)
   - Type safety throughout (Python type hints + Pydantic)
   - SQL injection not applicable (no direct DB access)

3. **Network Security**
   - HTTPS only (enforced by MCP client)
   - Bearer token authentication
   - No credential storage (user-managed)
...
1. **Additional CSV Columns**
   - Current: Team, Project, Title, Summary
   - Possible additions: Labels, Priority, State, Assignee, Due Date
   - Implementation: Update `IssueSpec` model + `create_issue` arguments

2. **Custom Validation Rules**
   - Pydantic validators can enforce:
     - Title length limits
     - Summary format requirements
     - Team/project naming conventions

3. **Alternative Input Formats**
   - JSON array import
   - YAML import
   - Direct database import
   - Implementation: New parser function returning `List[IssueSpec]`

4. **Batch Operations Beyond Creation**
   - Issue updates from CSV
   - Bulk status changes
   - Label assignment
   - Implementation: New MCP tool calls (`update_issue`, etc.)

## Use Cases Solved

### 1. **Sprint Planning Import**
- Export stories from Notion/Excel → CSV
- Bulk import to Linear
- Auto-create sprint-specific projects

### 2. **Migration from Other Systems**
- Export from Jira/GitHub Issues
- Transform to CSV format
- Import with project auto-creation

### 3. **Template-Based Story Creation**
- Define story templates in spreadsheet
- Duplicate/modify for variations
- Batch create entire feature sets

### 4. **Cross-Team Initiatives**
- PM creates master CSV with stories for multiple teams
- Single import creates issues across teams/projects
- Consistent formatting and structure

## Technical Constraints & Decisions

### Why CSV instead of direct Linear API?
- **MCP abstraction** - Future-proof against Linear API changes
- **Standardization** - Linear officially supports MCP
- **Simplicity** - No GraphQL schema management needed
...
### Why sequential issue creation?
- **Progress clarity** - Clear progress reporting for users
- **Error isolation** - Easier to debug when issues are created one-by-one
- **Trade-off** - Could parallelize for speed but would complicate error handling

### Why Python?
- **MCP SDK availability** - Official Python SDK (`mcp>=1.24.0`)
- **Data processing strength** - Excellent CSV/Pydantic ecosystem
- **CLI tooling** - Typer provides excellent UX
- **Async support** - Native async/await for MCP SSE connections

## Operational Characteristics

**Installation:** Single `pip install -e .` command
**Dependencies:** 4 core packages + Python 3.11+
**Configuration:** Environment variables or `.env` file
**Deployment:** CLI tool (no server/daemon needed)
**Logging:** Console output with progress indicators
**Error Recovery:** Continue-on-error mode with summary reporting

## Future Enhancement Opportunities

1. **Performance**
   - Parallel issue creation (with semaphore limiting)
   - HTTP/2 connection reuse
   - Batch issue creation API (if Linear adds support)

2. **Features**
   - Issue update/modification from CSV
   - Export Linear issues to CSV (reverse operation)
   - Interactive mode (select teams/projects with arrow keys)
   - Web UI for CSV upload

3. **Integration**
   - GitHub Actions workflow
   - CI/CD pipeline integration
   - Webhook triggers for automated imports
   - Slack notifications on completion

## Summary

Linear Issue Maker is a production-ready CLI tool that solves the bulk Linear issue creation problem through:
- Clean CSV-to-Linear pipeline
- Robust error handling and validation
- Efficient MCP protocol implementation
- Auto-project creation capability
- User-friendly progress reporting

The codebase is well-structured (~400 lines core logic), fully typed, and extensible for future enhancements. It successfully navigates the complexities of Linear's MCP server implementation while providing a simple, spreadsheet-based interface for users.
